...
const userDataController = async (req: Request, res: Response) => {
	if (!req.session.user) {
		...
		if(req.params.userId) {
			...
			['password', 'email', 'githubID'].forEach(key => delete censoredUser[key]);
			//check if user is a friend of req.session.user
			const isFriend = censoredUser!.friends.some((friend: any) => friend._id.toString() === req.session.user!._id.toString());
			if(!isFriend) {
